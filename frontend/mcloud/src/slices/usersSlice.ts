...
import type {UserProps, UserType} from '../models';
import axios from 'axios';


// Асинхронный thunk для получения данных пользователя из БД
export const fetchUserData = createAsyncThunk(
    'user/fetchUserData', async (userId: number) => {
    const response = await axios.get(`/api/users/${userId}/`); // Пример запроса
        // к API
...
const UsersSlice = createSlice({
    ...
    reducers: {
        loginSuccess: (state, action: PayloadAction<UserType>) => {
            ...
            localStorage.setItem('authState', JSON.stringify(state)); // Сохраняем
            // состояние при успешном входе
        },
        logoutSuccess: (state) => {
