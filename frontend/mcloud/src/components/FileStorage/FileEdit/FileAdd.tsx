...
import './FileAdd.css';
import type {FileAddProps} from "../../../models";
import { useSelector } from "react-redux";
import type {RootState} from "../../../store";


function FileAdd({ sendFile }: FileAddProps) {
    const file = useRef<HTMLInputElement | null>(null); // Реф на input для
    // выбора файлов
    const [fileChosen, setFileChosen] = useState<FileList | null>(null); // Состояние выбранных файлов
    const userId = useSelector((state: RootState) => state.users.loginUser?.id ?? 0); // Получаем ID
    // пользователя
