...
class User(AbstractUser):
    ...
    folder_name = models.CharField(max_length=100, verbose_name="Папка пользователя",
                                   blank=True)  # пусть к папке с файлами пользователя
...
class File(models.Model):
    ...
    def created_path_and_file_name(self, user_id, name):
        ...
        while File.objects.filter(user=user_id, file_name=unique_name).exists():
            ...
            print(f"base_name: {base_name}, counter: {counter}, extension: {extension}")
            counter += 1
