...
CREATE DATABASE diplom_cloud_server;
\q
exit
```
...
- Впишите следующий код, где смените `user` на вашего пользователя:
...
ExecStart=/home/ubuntu/mcloud_v2/backend/venv/bin/gunicorn --access-logfile - --workers 3 --bind unix:/home/ubuntu/diplom_MyCloud/backend/gunicorn.sock mycloud.wsgi:application
...
```bash
sudo nano /etc/nginx/sites-available/mcloud_v2
```
...
server {
    ...
    root /home/ubuntu/mcloud_v2/frontend/dist;
    ...
    location /api/ {
        ...
        proxy_pass http://unix:/home/ubuntu/mcloud_v2/backend/project.sock;
    }
    location /a/ {
        include proxy_params;
        proxy_pass http://unix:/home/ubuntu/mcloud_v2/backend/project.sock;
